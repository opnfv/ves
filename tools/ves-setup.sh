...
#.   Recommended sequence is to start with "bash ves-setup.sh collector" on 
#.   the collector host, then for each agent host:
#.     copy the env setup script (~/ves_env.sh) to ~/ves_env.sh on the host
...
function setup_collector() {
  ...
  echo "clone OPNFV VES"
  git clone https://gerrit.opnfv.org/gerrit/ves /tmp/ves

  log "setup influxdb container"
  mkdir /tmp/influxdb
  cp /tmp/ves/tests/onap-demo/blueprints/tosca-vnfd-onap-demo/infsetup.py \
    /tmp/influxdb/infsetup.py
  docker run -d --name=influxdb -p 8086:8086 \
    -v /tmp/influxdb/:/var/lib/influxdb influxdb

  log "setup InfluxDB datatabase"
  docker exec -d influxdb apt-get install -y python-influxdb
  docker exec -d influxdb python /var/lib/influxdb/infsetup.py

  log "install grafana"
  docker run -d --name grafana -p 3000:3000 grafana/grafana
  status=$(docker inspect grafana | jq -r '.[0].State.Status')
...
EOF
  ...
  curl -H "Accept: application/json" -H "Content-type: application/json" \
    -X POST -d @tosca-vnfd-onap-demo/Dashboard.json \
  ...
  docker run -d -v /tmp/ves:/opt/ves --name=ves-collector -p 30000:30000 \
    ubuntu:xenial

  # Note below: python (2.7) is required due to dependency on module 'ConfigParser'
  docker exec -d ves-collector apt-get install -y python python-jsonschema
  docker exec -d ves-collector \
    'nohup python /opt/ves/evel-test-collector/code/collector/monitor.py \
