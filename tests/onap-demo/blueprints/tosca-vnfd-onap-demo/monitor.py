...
def save_event(body):
  ...
  agent = jobj['event']['commonEventHeader']['reportingEntityName']
  if "virt" in agent: 
    agent = "computehost"
  if "VDU1" in agent: 
    agent = "webserver_1"
  ...
  if 'measurementsForVfScalingFields' in jobj['event']:
    ...
    if 'cpuUsageArray' in jobj['event']['measurementsForVfScalingFields']:
      ...
      pdata = 'cpu,system={},type=ubuntu,cpuid={} cpuuser={},cpusystem={},cpuidle={}'.format(agent,jobj['event']['measurementsForVfScalingFields']['cpuUsageArray'][0]['cpuIdentifier'], aggregateCpuUsageUser,aggregateCpuUsageSystem,aggregateCpuIdle)
      print(pdata)
    ...
    if 'vNicPerformanceArray' in jobj['event']['measurementsForVfScalingFields']:
      ...
      for vnic in e.event.measurementsForVfScalingFields.vNicPerformanceArray:
        pdata = 'vnic,system={},type=ubuntu,vnicid={} txoctets={},rxpacketsacc={},rxoctetsacc={},txpacketacc={}'.format(agent,vnic.vNicIdentifier,vnic.transmittedOctetsAccumulated,vnic.receivedTotalPacketsAccumulated,vnic.receivedOctetsAccumulated,vnic.transmittedTotalPacketsAccumulated)
        print(pdata)
        r = requests.post("http://localhost:8086/write?db=veseventsdb", data=pdata, headers={'Content-Type': 'text/plain'})
        if r.status_code != 204:
