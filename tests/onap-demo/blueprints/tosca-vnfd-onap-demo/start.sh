...
setup_agent () {
  ...
  cd ves
  git pull https://gerrit.opnfv.org/gerrit/ves refs/changes/47/35247/10
...
setup_monitor () {
  ...
  curl -H "Accept: application/json" -H "Content-type: application/json" -X POST -d '{"name":"VESEvents","type":"influxdb","access":"direct","url":"http://localhost:8080","password":"root","user":"root","database":"veseventsdb","basicAuth":false,"basicAuthUser":"","basicAuthPassword":"","withCredentials":false,"isDefault":false,"jsonData":null}' \
    http://admin:admin@localhost:3000/api/datasources

  echo "$0: Import Dashboard.json into grafana"
  curl -H "Accept: application/json" -H "Content-type: application/json" -X POST -d @Dashboard.json http://admin:admin@localhost:3000/api/dashboards/db	
