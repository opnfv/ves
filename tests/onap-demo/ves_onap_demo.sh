...
#     user: for host actions, ssh user on hosts
...
#
#   To access the InfluxDB and Grafana dashboards,
#   Add to your ~/.ssh/config file before ssh'ing to the jumphost. This guide
#   currently assumes you are running a virtual Apex install:
#   Host *
#     AddressFamily inet
#
...
ssh_to_agent() {
  if [[ $1 == 5 ]]; then
    ssh -t -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $3@$2
  else
    get_vdu_ip $1
...
start_monitor() {
  echo "$0: $(date) start Monitor in VDU5 at ${vdu_ip[5]}"
  scp -i ~/onap-demo -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no blueprints/tosca-vnfd-onap-demo/start.sh ubuntu@${vdu_ip[5]}:/home/ubuntu/start.sh
  scp -i ~/onap-demo -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no blueprints/tosca-vnfd-onap-demo/monitor.py ubuntu@${vdu_ip[5]}:/home/ubuntu/monitor.py
  ssh -i ~/onap-demo -x -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ubuntu@${vdu_ip[5]} "nohup bash /home/ubuntu/start.sh monitor ${vdu_id[1]} ${vdu_id[2]} ${vdu_id[3]} ${vdu_id[4]} hello world > /dev/null 2>&1 &"
}
...
get_vdu_ip () {
  ...
  echo "$0: $(date) find VM IP for VDU$1"
